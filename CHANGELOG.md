...
- **Code block rows rendered a cell narrower than the fence rows around them** - Syntect's line iterator keeps the trailing newline, and it was going into the span verbatim. `unicode-width` counts `\n` as a cell, so every code row measured one wider than it drew and the background padding came up one short, leaving a ragged right edge visible whenever the block background was on ([#75](https://github.com/Epistates/treemd/issues/75))
